...
function RecurrenceRule(rule) {
  ...
  if (typeof rule === 'string' || rule instanceof String) {
    this.rule = rrulestr(rule); //Returns an RRule or an RRuleSet
    return;
  }
  if (rule instanceof RRule || rule instanceof RRuleSet) {
    console.log(rule);
    this.rule = rule;
    return;
  }
  ...
  if (typeof rule === 'object') {
    this.rule = new RRule(rule);
    return;
  }

  throw new Error(
    'Invalid input for RecurrenceRule. Must be iCal string, RRule options object or RRule/RRuleSet'
  );
}
